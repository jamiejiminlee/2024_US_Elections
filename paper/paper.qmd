...
The predictive variables is 'state' 
```{r}
#| echo: false
#| warning: false
#| message: false
...
ggplot(clean_poll_data, aes(x = state)) +
  ...
  labs(
    title = "Distribution of State Variable in Polling Data",
    x = "State",
...
ggplot(clean_poll_data, aes(x = end_date)) +
  ...
  labs(
    title = "Distribution of Polling End Dates",
    x = "End Date",
    y = "Count of Polls"
  ) +
...
ggplot(clean_poll_data, aes(x = factor(pollster))) +  # Convert to factor to ensure proper categorization
  ...
  labs(
       x = "Count of Polls",
       y = "Pollster") +  # Notice the labels are switched for horizontal orientation
  theme_minimal() +
  coord_flip() +  # Flip the coordinates for a horizontal bar graph
  theme(axis.text.y = element_text(angle = 0, hjust = 1))  # Adjust y-axis text if necessary
```
...
- $\mu_i$ is the mean of the Beta distribution, reflecting the expected support proportion.
- $\phi$ is the precision parameter, indicating the variability around the mean.
