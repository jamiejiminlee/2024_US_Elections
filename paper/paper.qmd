...
In this section, we outline the two key predictor variables used in our analysis: 'state', which captures the geographical context of the polling data, and 'end_date', which reflects the timing of the polls relative to the election, allowing us to assess trends in voter support over ti
#### State
...
## Model Setup
In this section, we outline the statistical models employed to predict the probabilities of victory for Donald Trump and Kamala Harris in the 2024 presidential election within key battleground states. Our analysis utilizes a Bayesian framework, which allows us to incorporate uncertainty and prior information into the modeling process, yielding more robust predictions. The primary estimand for both models is the probability of each candidate's victory, expressed through a logistic regression approach. By examining polling data and its temporal dynamics, we aim to capture the evolving nature of voter support leading up to the election. We present separate subsections for the models of Donald Trump and Kamala Harris, detailing the structure, variables, and assumptions underlying each model. This division allows for a focused discussion on how each model has been tailored to reflect the unique electoral contexts and polling dynamics for each candidate. 
...
The model summary for Kamala Harris indicates that the intercept estimate is $-0.1508$ with a standard error of $0.0279$. This intercept value suggests a negative baseline log-odds for her victory, reflecting the starting point of her support in the absence of any other predictors. The model shows that the coefficients for the natural spline terms ($nsendate_{num}$) are particularly informative, with the first spline term ($nsendate_{num\_df41}$) yielding a positive estimate of $0.1225$, suggesting that as time progresses, Harris’s probability of winning increases, particularly during key moments captured by the polling data. This is also reflected in the confidence intervals, with the upper limit of the confidence interval for this term reaching $0.1903$. In contrast, the estimates for the second spline term ($nsendate_{num\_df42}$) indicate a smaller effect, suggesting fluctuations in her support that may warrant further investigation.
