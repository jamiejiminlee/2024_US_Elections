...
The analysis, modeling, and visualizations of this paper were conducted using the programming language `R` [@citeR] and the following packages: `tidyverse` @Tidyverse , `janitor` @Janitor for data cleaning, `kableExtra`@kableExtra for dynamic report generation and table formatting, `modelsummary` @Modelsummary for summarizing models, `usmap` @usmap for mapping U.S. states, `arrow` @arrow for data integration with Apache Arrow, `lubridate` @Lubridate for date handling, `rstanarm` @rstanarm and `brms` @brms for Bayesian regression modeling, `Matrix` @Matrix for handling sparse and dense matrices, `splines` @splines for regression splines, `caret` @caret for machine learning support, and `broom.mixed`  @broommixed for tidying mixed models.
