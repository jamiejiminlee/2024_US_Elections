...
subtitle: "A Bayesian Approach Reveals Strong Support for Donald Trump Across Key States, Highlighting Critical Trends in Voter Preferences Leading Up to Election Day"
author: 
  - Jimin Lee
  - Sarah Ding
  - Xiyan Chen
...
date: today
date-format: long
abstract: "This paper presents a predictive model for the 2024 United States Presidential Election using state-level polling data, with a Bayesian approach to estimate the outcomes between Donald Trump and Kamala Harris. By focusing on high-quality polls from reputable sources, we aggregated data to predict the likely winner in each state. Our Bayesian hierarchical model, incorporating time-dependent splines and state-specific random effects, captures shifts in candidate support over time. The results underscore Trump’s advantage in critical swing states such as Florida, Ohio, and North Carolina, positioning him as the predicted overall winner of the 2024 election. We discuss the model’s performance, accuracy, and limitations, reflecting on the challenges inherent in electoral forecasting."
format: pdf
...
# Introduction
This paper develops a predictive model for the 2024 US Presidential Election using state-level polling data [@FiveThirtyEight] to forecast the likely winner between Donald Trump and Kamala Harris. We aggregate high-quality poll data to create a Bayesian logistic regression model that estimates the probability of a Trump or Harris victory in each state. By comparing polling percentages for both candidates, we aim to predict election outcomes based on voter support patterns across the states.
...
Our model utilizes a Bayesian approach with a hierarchical structure to predict election outcomes by comparing the average polling percentages for Trump and Harris across states. This methodology incorporates time-dependent splines and state-specific random effects, which allow for capturing variations in support trends over time. Swing states emerge as critical battlegrounds, where polling percentages are closely contested and could influence the final election result.

Accurate election predictions provide valuable insights into voter dynamics and help political analysts, campaigns, and the public anticipate electoral outcomes. By focusing on high-quality polling data, our model enhances the reliability of predictions and highlights key regions where voter sentiment may shift, ultimately affecting the election.

The results of our model predict a victory for Donald Trump in the 2024 U.S. Presidential Election. Key battleground states, such as Florida, Ohio, and North Carolina, lean toward Trump, emphasizing his advantage in critical regions. Our analysis points to Trump as the probable overall winner, with swing states playing a decisive role in determining the final outcome.
...
- $ f(\text{end\_date\_num}\_i) $ is a natural spline function of time (measured as $ \text{end\_date\_num} $), with 4 degrees of freedom, capturing the non-linear time trends in support.
...
# Display state predictions i n table
...
ggplot(state_predictions_long, aes(x = reorder(state, Probability), y = Probability * 100, fill = Candidate)) +
  ...
  labs(title = "Predicted Winning Percentages by State on Election Day", 
       x = "State", 
