...
Beyond data and modeling, several external factors can influence the actual outcome of the election. Recency bias is one of the external factors, and it can significantly shape public perception and polling responses. Many people expect Trump to win due to his unexpected victory in 2016 and his narrow loss in 2020, despite lagging in the polls. This bias reflects a tendency to focus on recent, high-profile events while overlooking broader historical trends. For instance, in 2012, Barack Obama not only won but outperformed polling predictions, a fact that may be forgotten amidst recent election narratives [@discussion-article3]. Such a bias can affect both how people respond to polls and how pollsters interpret results, potentially leading to an overemphasis on recent events. Given that our model depends on polling data, any systematic bias from these recency expectations can skew predictions, making it challenging to account for potential polling errors.

Social desirability bias and non-response bias add further layers of uncertainty, particularly regarding Trump’s support base. Some voters may be hesitant to openly express support for a conservative candidate due to social stigma, leading to an under-reporting of Trump support in polls. This effect may be stronger in regions where conservative views are less socially accepted. Additionally, Trump’s supporters often include individuals with lower social trust and civic engagement, making them less likely to participate in polls altogether [@discussion-article3]. If specific groups systematically avoid participation, polling data may under-represent these voters, potentially underestimating Trump’s support in our model. Non-response bias is difficult to quantify, introducing additional uncertainty to our predictions.

Harris, on the other hand, may face challenges like the Bradley effect and gender-related bias. As a Black female candidate, Harris could encounter the Bradley effect, where some voters indicate indecision rather than openly stating that they will not support a minority candidate. Historically, undecided voters have leaned against Black candidates in the actual vote count, despite neutral polling responses. Additionally, the 2016 election demonstrated that undecided voters could heavily swing against female candidates, as seen with Hillary Clinton [@discussion-article3]. This trend could similarly impact Harris’s results if undecided voters ultimately decide against her. Consequently, our model may overestimate Harris’s support based on polling data alone, leading to potential inaccuracies in prediction.

These biases are inherently challenging to measure or adjust for, as they reflect deeper societal attitudes and individual decision-making patterns. While pollsters may attempt to correct for non-response or social desirability bias, such adjustments are often based on assumptions that cannot fully capture the complex social dynamics at play. Thus, our model’s predictions should be interpreted with caution, recognizing that these biases may cause unexpected shifts in voter behavior that go beyond what data-driven forecasts can capture.
...
Building on our discussion of the New York Times/Siena College Poll, we now present an idealized methodology that aims to enhance the accuracy and reliability of our predictions. This approach incorporates best practices and innovative techniques to ensure a comprehensive prediction of voter behavior in the upcoming 2024 U.S. presidential election. To demonstrate our idealized methodology, we generated a survey on the 2024 US Presidential election, which can be accessed via the the URL provided in [@US-election-survey] under [References -@sec-references].
