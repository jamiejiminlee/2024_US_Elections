...
The 2024 United States Presidential Election is anticipated to be a pivotal moment in American political history, particularly as battleground states emerge as key determinants of the electoral outcome. With candidates Donald Trump and Kamala Harris intensifying their campaigns, a thorough understanding of voter preferences in these critical regions becomes essential for forecasting election results. This paper leverages state-level polling data from Project 538 [@FiveThirtyEight] and employs the statistical programming language R [@citeR] to construct a predictive model that estimates the likelihood of victory for each candidate across several crucial battleground states, including Arizona, Georgia, Michigan, Nevada, North Carolina, Pennsylvania, and Wisconsin.

The primary estimand of this study is the probability of winning for either Donald Trump or Kamala Harris in these battleground states, derived from aggregated polling averages. Our analysis utilizes a Bayesian hierarchical approach combined with natural splines to accurately capture the temporal dynamics of voter sentiment, enabling us to estimate winning probabilities in these decisive areas. By comparing the predicted winning probabilities for both candidates, we can ascertain the likely winner based on prevailing voter support trends. Our findings indicate a competitive landscape, with Kamala Harris projected to secure victories in six of the seven battleground states analyzed, including key states like Michigan and Nevada, where voter sentiment appears to favor her candidacy. However, Donald Trump is also predicted to maintain a competitive edge in Arizona, showcasing the variability in voter preferences across these critical regions. The predicted winner is determined by comparing the estimated probabilities for each candidate, highlighting Harris’s overall advantage leading into the election.

Understanding these predictions is crucial as they not only forecast the likely outcome of the U.S. election but also help society and the economy to better understand the current state of the US political climate. The implications of these findings extend beyond mere predictions, providing a framework for analyzing how shifts in voter sentiment may occur as the election draws near. The remainder of this paper is organized as follows: Section @sec-data details the data and variables utilized in the analysis; Section @sec-model outlines the model setup and estimation strategies employed; Section @sec-results presents the results, including visualizations of the predicted election outcomes; and Section @sec-discussion discusses the implications and limitations of our findings.
...
#| fig-cap: "Distribution of Observations for Each State in Polling Data. Illustrates the number of polling observations across key battleground statesn. Notably, Pennsylvania has the highest count of observations, suggesting a more robust polling presence, while Nevada shows fewer observations. "
